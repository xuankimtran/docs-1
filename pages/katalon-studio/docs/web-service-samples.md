...
Here, we want to send the user information with dynamic data, including username, password, age, gender, and avatar to the server to create new accounts. To do so, we use the `${<variable_name>}` syntax as a placeholder in the HTTP body as follows. To learn more about setting parameter in a web service object, you can refer to this document: [Parameterize a web service object](https://docs.katalon.com/katalon-studio/docs/parameterize-a-web-service-object.html#query-parameters).
...
- Execute verification snippets available in the **Verification** tab of the POST request.
