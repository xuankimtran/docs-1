...
---
**AWS Device Farm Integration** is a Proof of Concept (PoC) built for executing Katalon Studio tests on [AWS Device Farm](https://aws.amazon.com/device-farm/). AWS Device Farm only supports running tests using supported frameworks such as working with Appium and AWS Device Farm; hence, Katalon users cannot execute their tests on AWS Device farm directly. See AWS documentation on [Working with Appium and AWS Device Farm](https://docs.aws.amazon.com/devicefarm/latest/developerguide/test-types-appium.html).
...
1. Prepare Katalon tested test cases, test suites that can run successfully on local device.
    ...
    > For Mobile test, a test should start with Start Existing Application keyword because AWS Device Farm already installs the application on tested device before every run. See [[Mobile] Start Existing Application](https://docs.katalon.com/katalon-studio/docs/mobile-keyword-start-existing-apps.html).
...
4. In `Configure` Step, select `Appium Java JUnit` in test type drop-down. Upload the `zip-with-dependencies.zip` that was created from Maven in the previous step then click `Next`.
...
5. In `Select devices` Step, choose a suitable device pool then click `Next`.
6. In `Specify device state` Step, review other settings and change when needed then click `Next`
7. In `Review and start` run Step, review all of configurations one last time then click `Confirm and Start Run`.
...
8. A run already starts, we can click on test run name and a specificed device to view the test status
